...
public class SaleInvoDeta {
    ...
    @Builder.Default
    private BigDecimal quantia_sainde = BigDecimal.ZERO;  // Cantidad

    @Builder.Default
    private BigDecimal discoun_sainde = BigDecimal.ZERO;  // Descuento (porcentaje)

    @Builder.Default
    private BigDecimal pricea_sainde = BigDecimal.ZERO;   // Precio base

    private BigDecimal priceb_sainde;                     // Precio final con descuento

    @Builder.Default
    private BigDecimal value_sainde = BigDecimal.ZERO;     // Subtotal final

    // ... otros campos (quantib_sainde, nandina_sainde, etc.)

    // ========== LÓGICA DE CÁLCULO ==========

    /**
     * Calcula y actualiza automáticamente:
     * 1. priceb_sainde (precio con descuento)
     * 2. value_sainde (subtotal final)
     */
